...
def pad_sequences(data, max_length):
    ...
    for sentence, labels in data:
        if len(sentence) > max_length:
            ...
            new_labels = labels + [zero_label] * (max_length - len(labels))
            mask = [True] * len(sentence) + [False] * (max_length - len(labels))
...
class RNNModel(NERModel):
    ...
    def add_embedding(self):
        ...
        embedding_lookup = tf.nn.embedding_lookup(pretrained, self.input_placeholder)
        embeddings = tf.reshape(embedding_lookup,
    ...
    def add_prediction_op(self):
        ...
        with tf.variable_scope("RNN"):
            h_t = tf.zeros((batch_size, self.config.hidden_size))
            ...
            for time_step in range(self.max_length):
                ...
                o_drop = tf.nn.dropout(o_t, dropout_rate)
                p = tf.matmul(o_drop, U) + b2
    ...
    def add_loss_op(self, preds):
        ...
        masked = tf.boolean_mask(cross_entropy, self.mask_placeholder)
