...
def forward_backward_prop(X, labels, params, dimensions):
    ...
    yp = softmax(h.dot(W2) + b2)
    ce = inner1d(labels, -np.log(yp))
    cost = np.sum(ce)
    # END YOUR CODE
    ...
    gradW2 = h.T.dot(gradCe)
    gradb2 = np.sum(gradCe, axis=0)

    dlayer1 = gradCe.dot(W2.T) * sigmoid_grad(h)
    gradW1 = X.T.dot(dlayer1)
