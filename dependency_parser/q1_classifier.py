...
class SoftmaxModel(Model):
    ...
    def add_prediction_op(self):
        ...
        """
        weight = tf.Variable(tf.zeros(shape=(self.config.n_features, self.config.n_classes)), name='weight')
        bias = tf.Variable(tf.zeros(shape=(self.config.n_classes,)), name='bias')

        z = tf.matmul(self.input_placeholder, weight) + bias
        pred = softmax(z)
