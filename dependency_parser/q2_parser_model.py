...
class Config(object):
    ...
    dropout = 0.4  # (p_drop in the handout)
    embed_size = 50
    hidden_size = 200
    batch_size = 1024
    n_epochs = 10
...
class ParserModel(Model):
    ...
    def add_placeholders(self):
        ...
        self.labels_placeholder = tf.placeholder(tf.float32, (None, self.config.n_classes), 'labels')
    ...
    def add_embedding(self):
        ...
        raw_embedding = tf.nn.embedding_lookup(pretrained_embeddings, self.input_placeholder)

        embeddings = tf.reshape(raw_embedding, (-1, self.config.n_features * self.config.embed_size), 'embeddings')
    ...
    def add_loss_op(self, pred):
        ...
        """
        return tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits(labels=self.labels_placeholder,
                                                                      logits=pred))
