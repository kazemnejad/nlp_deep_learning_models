...
def make_windowed_data(data, start, end, window_size=1):
    ...
    for sentence, labels in data:
        for i, label in zip(range(len(sentence)), labels):
            s_index = max(0, i - window_size)
            e_index = min(len(sentence) - 1, i + window_size)

            prefix = [start] * (window_size - (i - s_index))
            kernel = sentence[s_index:e_index + 1]
            postfix = [end] * (window_size - (e_index - i))
            window = prefix + kernel + postfix

            windowed_data.append((list(itertools.chain.from_iterable(window)), label))
...
class WindowModel(NERModel):
    ...
    def add_placeholders(self):
        ...
        """
        self.input_placeholder = tf.placeholder(tf.int32, (None, self.config.n_window_features), 'inputs')
    ...
    def add_embedding(self):
        ...
        # initial_embeddings = tf.nn.embedding_lookup(pretrained, self.input_placeholder)
        # embeddings = tf.reshape(initial_embeddings, (-1, self.config.n_window_features * self.config.embed_size))
        #
        # return embeddings
        ...
        embeddings = tf.reshape(emb_lookup, [-1, self.config.n_window_features * self.config.embed_size])
    ...
    def add_prediction_op(self):
        ...
        b1 = tf.zeros((self.config.hidden_size,), dtype=tf.float32)
        b2 = tf.zeros((self.config.n_classes,), dtype=tf.float32)
    ...
    def add_loss_op(self, pred):
        ...
        return tf.reduce_mean(tf.nn.sparse_softmax_cross_entropy_with_logits(logits=pred,
